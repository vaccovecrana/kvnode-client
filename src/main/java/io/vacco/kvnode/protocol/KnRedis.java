 ...
 */
public class KnRedis implements Closeable {

  /** Used for writing the data to the server. */
  private final KnEncoder writer;
  /** Used for reading responses from the server. */
  private final KnParser reader;
   ...
   */
  public KnRedis(Socket socket) throws IOException {
   ...
   * @throws IOException If a socket error occurs.
   */
  public KnRedis(Socket socket, int inputBufferSize, int outputBufferSize) throws IOException {
    this(
   ...
   */
  public KnRedis(InputStream inputStream, OutputStream outputStream) {
    this.reader = new KnParser(inputStream);
    this.writer = new KnEncoder(outputStream);
  }
   ...
   */
  public KnPipeline pipeline() {
    return new KnPipeline(writer, reader);
  }
